...
The complete API reference is available in `pkg/swagger/swagger.json`. This file contains all available endpoints, request/response formats, and model definitions.

**Important:** Never touch the generated swagger api documentation under `pkg/swagger/`. These are automatically generated by CI after committing.

### Common API Endpoints

When working with APIs, refer to the swagger documentation for:
...
- **Teams:**
  ...
  - `POST /teams/{id}/members/{username}/admin` - Toggle admin status

- **Users:**
  - `GET /users?s=<search>` - Search users by username, name or email
  - Requires `s` query parameter (can be empty string to get all users)
  - Returns array of `user.User` objects

- **Projects:**
  - `POST /projects` - Create a new project
  - `PUT /projects/{project}/users` - Share project with user
  - Response format depends on user discoverability settings

### Data Models

Key models defined in swagger:

- **models.Team:** Contains `members` array of `models.TeamUser`
- **models.TeamUser:** Member object with `id`, `name`, `username`, `email`, `admin`
- **models.TeamMember:** Request body for adding user to team (requires `username`)
- **user.User:** User information object

### Usage Guidelines

When writing API-related code:

1. **Always reference `pkg/swagger/swagger.json`** for endpoint URLs, parameters, and response formats
2. **Check authentication requirements** - most endpoints require JWT token in Authorization header
3. **Verify field names** - use exact field names from swagger definitions (e.g., `member['id']` not `member['userID']`)
4. **Handle pagination** - some endpoints may support pagination parameters
5. **Handle errors** - check HTTP status codes and error responses defined in swagger
6. **Test with real API** - use swagger or Postman to verify endpoints before implementing
...
Project-specific skills are stored in `.opencode/skills/` and contain specialized workflows and tools tailored to this project.

**Skill Directory Structure:**
- `.opencode/skills/` - Location of all project-specific skills
- Each skill follows the standard skill structure with SKILL.md and optional scripts/, references/, assets/ directories

**Available Skills:**
- `add-employees-batch` - Batch add multiple new employees to company team and create personal task projects

When users request functionality that matches a project skill's description, load and use that skill to provide specialized assistance.

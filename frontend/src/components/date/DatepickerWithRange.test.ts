...
describe('DatepickerWithRange predefined ranges', () => {
	...
	it('selects Last Week range', async () => {
		...
		;(wrapper.vm as unknown as {setDateRange: (range: string[]) => void}).setDateRange(['now/w-1w', 'now/w'])
	...
	it('selects Last Month range', async () => {
		const wrapper = mountPicker()
		;(wrapper.vm as unknown as {setDateRange: (range: string[]) => void}).setDateRange(['now/M-1M', 'now/M'])
		await wrapper.vm.$nextTick()
		const last = wrapper.emitted('update:modelValue')?.pop()?.[0]
